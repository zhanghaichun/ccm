...
union all 
(SELECT '23' flag, p.account_code_id, a.account_code_name, a.account_code_description,  p.invoice_id,    null ban_id, p.dispute_amount*-1,   p.description,    null date1,       null created_by, p.attachment_point_id FROM proposal p, dispute d,account_code a  WHERE p.dispute_id=d.id AND p.account_code_id=a.id  AND d.flag_shortpaid='Y'  AND p.invoice_item_id is NULL      AND p.dispute_amount is not NULL  AND p.dispute_amount!=0  AND p.rec_active_flag='Y'  AND p.proposal_flag='1');
...
BEGIN 
   IF ((OLD.invoice_status_id!=NEW.invoice_status_id) OR (OLD.rec_active_flag!=NEW.rec_active_flag AND NEW.rec_active_flag='N')) THEN 
...
--   IF (OLD.rec_active_flag!=NEW.rec_active_flag AND NEW.rec_active_flag='N') THEN 
--       UPDATE transaction_history SET rec_active_flag='N' WHERE invoice_id=NEW.id;
--   END IF;   
END;
...
BEGIN 
   IF (OLD.account_code_id!=NEW.account_code_id 
      ...
      OR (OLD.rec_active_flag!=NEW.rec_active_flag AND NEW.rec_active_flag='N')) THEN 
       INSERT INTO proposal_history 
       (proposal_id, old_account_code_id, old_payment_amount, old_dispute_amount, old_credit_amount, old_balance_amount, old_dispute_reason_id, new_account_code_id, new_payment_amount, new_dispute_amount, new_credit_amount, new_balance_amount, new_dispute_reason_id, created_timestamp, created_by, rec_active_flag) 
       VALUES 
       (NEW.id, OLD.account_code_id, OLD.payment_amount, OLD.dispute_amount, OLD.credit_amount, OLD.balance_amount, OLD.dispute_reason_id, NEW.account_code_id, NEW.payment_amount, NEW.dispute_amount, NEW.credit_amount, NEW.balance_amount, NEW.dispute_reason_id, NEW.modified_timestamp , NEW.modified_by, 'Y');
   END IF;

--   IF (OLD.rec_active_flag!=NEW.rec_active_flag AND NEW.rec_active_flag='N') THEN 
--       UPDATE proposal_history SET rec_active_flag='N' WHERE proposal_id=NEW.id;
--   END IF;
END;
...
BEGIN 
   IF (NEW.account_code_id IS NOT NULL
      ...
      OR NEW.dispute_reason_id  IS NOT NULL) THEN 
       ...
       (proposal_id, old_account_code_id, old_payment_amount, old_dispute_amount, old_credit_amount, old_balance_amount, old_dispute_reason_id, new_account_code_id, new_payment_amount, new_dispute_amount, new_credit_amount, new_balance_amount, new_dispute_reason_id, created_timestamp, created_by, rec_active_flag) 
       VALUES 
       (NEW.id, null, null, null, null, null, null, NEW.account_code_id, NEW.payment_amount, NEW.dispute_amount, NEW.credit_amount, NEW.balance_amount, NEW.dispute_reason_id, NEW.created_timestamp , NEW.created_by, 'Y');
   END IF;
END;
