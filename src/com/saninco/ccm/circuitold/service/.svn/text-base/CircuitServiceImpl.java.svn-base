...
import com.saninco.ccm.circuitold.dao.ICircuitDao;
...
public class CircuitServiceImpl implements ICircuitService  {
	private ICircuitDao circuitOldDao;
	private final Logger logger = Logger.getLogger(this.getClass());
	
	private static int DOWNLOAD_EXCEL_RECPERPAGE = 500;
	...
	public String searchCircuit(SearchCircuitVO searchCircuitVO) throws BPLException {
		...
		List<Object[]> l = null;
		try {
			l = circuitOldDao.searchCircuit(searchCircuitVO, SystemUtil.getCurrentUserId());
	...
	public String getCircuitsSearchTotalPageNo(SearchCircuitVO searchCircuitVO) throws BPLException {
		...
		long count = 0;
		try {
			count = circuitOldDao.getNumberOfCircuits(searchCircuitVO, SystemUtil.getCurrentUserId());
		} catch (RuntimeException e) {
			logger.error(CcmFormat.formatErrorLog(e));
			BPLException bpe = new BPLException(ErrorCodeConstants.EC_HIBERNATE_FAILED_ACCESS_DB);
			throw bpe;
	...
	public String getDataToExcel(SearchCircuitVO svo,String sheetName, String excelDirPath, List<String> titles) throws BPLException {
		...
		try {
			...
			int pageNumber = 0;
			long count = 0;
			count = circuitOldDao.getNumberOfCircuits(svo, SystemUtil.getCurrentUserId());
			...
			if (count <= DOWNLOAD_EXCEL_RECPERPAGE) {
				...
				list = circuitOldDao.getCircuitDataForExcel(svo, SystemUtil.getCurrentUserId());
				...
				for (int j = 0; j < totlePage; j++) {
					...
					if(pageNumber > 0){
						if(((j * DOWNLOAD_EXCEL_RECPERPAGE) % DOWNLOAD_EXCEL_MAXSIZE) == 0 ){
							...
							jExcelUtil.createWritableSheet(pageNumber, sheetName + (pageNumber + 1));
	...
	public String getTabs(SearchCircuitVO svo)throws BPLException{
		...
		List<String> l = null;
		try {
		l=circuitOldDao.getTabs(svo,  SystemUtil.getCurrentUserId());
	...
	public String getCircuitDateHyperlink(SearchCircuitVO svo)throws BPLException{
		...
		List<String> l = null;
		try {
		l=circuitOldDao.getCircuitDateHyperlink(svo,  SystemUtil.getCurrentUserId());
		sb.append(getListJson(l));
		} catch (RuntimeException e) {
			logger.error(CcmFormat.formatErrorLog(e));
			BPLException bpe = new BPLException(ErrorCodeConstants.EC_HIBERNATE_FAILED_ACCESS_DB);
	...
	public void setCircuitOldDao(ICircuitDao circuitOldDao) {
		this.circuitOldDao = circuitOldDao;
	}
